defmodule Puzzle.Prisoner do
  ...
  def handle_call({:visit_room, light_on_at_arrival?}, _from, state) do
    ...
    {light_at_exit, known_count} =
      case {state.day, light_on_at_arrival?, state.known_count, state.id == 100} do
        {1, true, 0, true} -> 
          {false, 1}
        {_, true, 0, true} -> 
          {false, 2}
        {_, true, _, true} -> 
          {false, state.known_count + 1}  
        {_, false, 0, false} ->
          {true, 1}
        _ -> 
          {light_on_at_arrival?, state.known_count}
      end
